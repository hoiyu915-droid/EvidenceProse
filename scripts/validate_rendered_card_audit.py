...
from validate_rendered_card_source_closure import (
    METHOD as SOURCE_METHOD,
    validate_card as validate_source_surface_card,
)
...
FAIL=set(RCA_POLICY["verdict_mapping"]["material_failure_verdicts"])
...
def _validate_semantic_packet(
    ...
    source_ids=_string_list(
        packet.get("source_binding_ids"),
        f"{prefix}.expected_semantic_packet.source_binding_ids",
        errors,
        nonempty=True,
    )
    ...
    for i,item in enumerate(expected_inventory):
        ...
        if not txt(node_id):
            add(errors,f"{q}.expected_node_id required")
        else:
    ...
    for i,item in enumerate(expected_annotations):
        ...
        if not isinstance(role,str) or role not in set(SOURCE_POLICY_ANNOTATION_ROLES):
...
def source_locator_ids(card:dict[str,Any],errors:list[str]|None=None) -> set[str]:
    ...
    for key in ("content_node_checks","expected_content_checks","evidence_annotation_checks"):
        ...
        for i,item in enumerate(checks):
            ...
            for j,locator in enumerate(locators):
                sid=_source_locator_id(
                    locator,
                    f"source_surface_reconciliation.{key}[{i}].source_locators[{j}]",
                    local_errors,
                )
                if sid:
                    out.add(sid)
    ...
    source_structure=(
        visual.get("source_structure",{}) if isinstance(visual,dict) else {}
    )
    locators=(
        source_structure.get("source_locators",[])
    ...
    if isinstance(locators,list):
        for i,locator in enumerate(locators):
            sid=_source_locator_id(
                locator,
                f"visual_semantic_reconciliation.source_structure.source_locators[{i}]",
                local_errors,
            )
            if sid:
                out.add(sid)
...
def file_sha(base,rel):
    if base is None or not txt(rel):
        return None
    p=Path(rel)
...
def finding_map(items,prefix,errors):
    if not isinstance(items,list):
        add(errors,f"{prefix} must be array")
        return {}
    out={}
    for i,f in enumerate(items):
        p=f"{prefix}[{i}]"
        if not isinstance(f,dict):
            add(errors,f"{p} must be object")
            continue
        fid=f.get("finding_id")
        if not txt(fid):
            add(errors,f"{p}.finding_id required")
        elif fid in out:
            add(errors,f"{prefix} duplicate finding_id {fid}")
        else:
            out[fid]=f
        m=f.get("materiality")
        if not isinstance(m,str) or m not in {
            ...
            add(errors,f"{p}.materiality invalid")
        support=f.get("source_support",[])
        if (
            isinstance(m,str)
            and m in {"material","blocking_unverifiable"}
            and (
                not isinstance(support,list)
                or not any(txt(x) for x in support if isinstance(x,str))
            )
        ):
            add(errors,f"{p} material finding requires source_support")
        rp=f.get("repair_prescription")
        if not isinstance(rp,dict) or not txt(rp.get("instruction")):
            add(errors,f"{p} repair_prescription required")
    return out

def linked(fid,findings,prefix,errors):
    if not txt(fid) or fid not in findings:
        add(errors,f"{prefix} requires linked finding")
        return
    if findings[fid].get("axis")!="VISUAL_SEMANTICS":
        add(errors,f"{prefix} finding must be VISUAL_SEMANTICS")

def visual_status(rec,findings,prefix,errors):
    if not isinstance(rec,dict):
        add(errors,f"{prefix} reconciliation required")
        return "unverifiable"
    src=rec.get("source_structure",{})
    if not isinstance(src,dict):
        src={}
    kind=src.get("source_kind")
    if src.get("structure_required") is not True:
        add(errors,f"{prefix} source structure required")
    if not isinstance(kind,str) or kind not in {
        ...
        add(errors,f"{prefix} source_kind invalid")
    ...
    if not isinstance(source_locators,list):
        ...
        source_locators=[]
    if not any(txt(x) for x in source_locators if isinstance(x,str)):
        add(errors,f"{prefix} source locators required")
    for i,locator in enumerate(source_locators):
        _source_locator_id(locator,f"{prefix}.source_structure.source_locators[{i}]",errors)
    if (
        isinstance(kind,str)
        and kind in {"figure","table","mixed"}
        and src.get("figure_or_table_directly_inspected") is not True
    ):
        add(errors,f"{prefix} figure/table-derived structure requires direct inspection")
    if src.get("expected_graph_derived") is not True:
        add(errors,f"{prefix} expected graph must be derived")
    og,eg=rec.get("observed_graph"),rec.get("expected_graph")
    if not isinstance(og,dict) or not isinstance(eg,dict):
        add(errors,f"{prefix} observed/expected graph required")
        return "unverifiable"
    if rec.get("observed_graph_digest")!=jd(og):
        add(errors,f"{prefix} observed_graph_digest mismatch")
    if rec.get("expected_graph_digest")!=jd(eg):
        add(errors,f"{prefix} expected_graph_digest mismatch")
    ...
    ec=rec.get("edge_checks",[])
    eids=check_ids(ec,"edge_id","edge_checks")
    if sorted(eids)!=sorted(edges):
        add(errors,f"{prefix} every observed material edge must be dispositioned exactly once")
    for x in ec if isinstance(ec,list) else []:
        ...
        if bucket=="block":
            unv=True
            linked(x.get("finding_id"),findings,f"{prefix} edge {x.get('edge_id')}",errors)
        elif bucket=="fail":
            bad=True
            linked(x.get("finding_id"),findings,f"{prefix} edge {x.get('edge_id')}",errors)
        elif bucket=="warning":
            ...
            add(errors,f"{prefix} edge status invalid")
    rc=rec.get("expected_relation_checks",[])
    rids=check_ids(rc,"relation_id","expected_relation_checks")
    if sorted(rids)!=sorted(rels):
        add(errors,f"{prefix} every expected material relation must be covered exactly once")
    for x in rc if isinstance(rc,list) else []:
        ...
        if bucket=="block":
            unv=True
            linked(x.get("finding_id"),findings,f"{prefix} relation {x.get('relation_id')}",errors)
        elif bucket=="fail":
            bad=True
            linked(x.get("finding_id"),findings,f"{prefix} relation {x.get('relation_id')}",errors)
        elif bucket=="warning":
            ...
            add(errors,f"{prefix} relation status invalid")
    bc=rec.get("branch_point_checks",[])
    bids=check_ids(bc,"branch_id","branch_point_checks")
    if sorted(bids)!=sorted(branches):
        add(errors,f"{prefix} every material branch point must be checked exactly once")
    tc=rec.get("terminal_state_checks",[])
    tids=check_ids(tc,"terminal_id","terminal_state_checks")
    if sorted(tids)!=sorted(terms):
        add(errors,f"{prefix} every material terminal state must be checked exactly once")
    topology_check_groups=(
        ("branch",bc,"branch_id",BRANCH_STATUSES),
        ("terminal",tc,"terminal_id",TERMINAL_STATUSES),
    )
    for label,items,key,statuses in topology_check_groups:
        for x in items if isinstance(items,list) else []:
            ...
            if bucket=="block":
                unv=True
                linked(x.get("finding_id"),findings,f"{prefix} {label} {x.get(key)}",errors)
            elif bucket=="fail":
                bad=True
                linked(x.get("finding_id"),findings,f"{prefix} {label} {x.get(key)}",errors)
            elif bucket=="warning":
                ...
                add(errors,f"{prefix} {label} status invalid")
    pc=rec.get("role_partition_checks",[])
    pids=check_ids(pc,"partition_id","role_partition_checks")
    if sorted(pids)!=sorted(partitions):
        add(errors,f"{prefix} every material role partition must be checked exactly once")
    for x in pc if isinstance(pc,list) else []:
        ...
        if bucket=="block":
            ...
            linked(
                x.get("finding_id"),
                findings,
                f"{prefix} role partition {x.get('partition_id')}",
                errors,
            )
        elif bucket=="fail":
            bad=True
            ...
            add(errors,f"{prefix} role partition status invalid")
    tv=rec.get("text_visual_consistency",{})
    st=tv.get("status") if isinstance(tv,dict) else None
    text_visual_bucket=_status_bucket(st,TEXT_VISUAL_STATUSES)
    if text_visual_bucket=="block":
        unv=True
        linked(tv.get("finding_id"),findings,f"{prefix} text/visual failure",errors)
    elif text_visual_bucket=="fail":
        bad=True
        linked(tv.get("finding_id"),findings,f"{prefix} text/visual failure",errors)
    elif text_visual_bucket=="warning":
        warn=True
    elif text_visual_bucket!="pass":
        add(errors,f"{prefix} text_visual_consistency invalid")
    comp=rec.get("topology_completion",{})
    if not isinstance(comp,dict):
        add(errors,f"{prefix} topology_completion must be object")
        comp={}
    required={
        "all_observed_material_edges_dispositioned":sorted(eids)==sorted(edges),
        "all_expected_material_relations_covered":sorted(rids)==sorted(rels),
        "all_material_branch_points_checked":sorted(bids)==sorted(branches),
        "all_material_terminal_states_checked":sorted(tids)==sorted(terms),
        "all_material_role_partitions_checked":sorted(pids)==sorted(partitions),
        "text_visual_consistency_checked":text_visual_bucket is not None,
        "concept_presence_shortcut_not_used":True,
    }
    for k,v in required.items():
        if comp.get(k) is not v:
            add(errors,f"{prefix} topology_completion.{k} must be {v}")
    if comp.get("concept_presence_shortcut_not_used") is not True:
        add(errors,f"{prefix} concept-presence shortcut is forbidden")
    return "unverifiable" if unv else "fail" if bad else "warning" if warn else "pass"

def validate_ticket(t,errors):
    if not isinstance(t,dict):
        add(errors,"repair ticket must be object")
        return
    p=f"repair ticket {t.get('ticket_id')}"
    if not isinstance(t.get("verdict"),str) or t.get("verdict") not in FAIL:
        add(errors,f"{p} verdict invalid")
    if not txt(t.get("card_title")) or not txt(t.get("human_repair_text")):
        add(errors,f"{p} card_title and human_repair_text required for every failure")
    if not t.get("action_plan"):
        add(errors,f"{p} action_plan required")
    s=t.get("substantial_change")
    if not isinstance(s,dict):
        add(errors,f"{p} substantial_change required")
        return
    total,changed=s.get("total_semantic_weight"),s.get("changed_semantic_weight")
    if (
        not isinstance(total,(int,float))
        or not isinstance(changed,(int,float))
        or total<0
        or changed<0
        or (total and changed>total)
    ):
        add(errors,f"{p} semantic weights invalid")
        return
    frac=None if total<=0 else changed/total
    fraction_value=s.get("estimated_weighted_semantic_fraction")
    if frac is not None and (
        not isinstance(fraction_value,(int,float))
        or abs(fraction_value-frac)>1e-9
    ):
        add(errors,f"{p} fraction must equal changed/total")
    triggered=bool(s.get("structural_triggers")) or (total>=10 and frac is not None and frac>=.10)
    if s.get("triggered") is not triggered:
        add(errors,f"{p} triggered does not match policy")
    if triggered:
        if s.get("replacement_required") is not True:
            add(errors,f"{p} substantial repair requires replacement")
        if s.get("replacement_status")=="SUPPORTED":
            rm=s.get("replacement_material")
            if (
                not isinstance(rm,dict)
                or rm.get("presentation")!="writing_block"
                or not txt(rm.get("text"))
            ):
                add(errors,f"{p} SUPPORTED replacement requires writing_block text")
        elif s.get("replacement_status")!="BLOCKED_NO_SUPPORTED_MATERIAL":
            add(errors,f"{p} replacement_status invalid")
    elif (
        s.get("replacement_required") is not False
        or s.get("replacement_status")!="NOT_REQUIRED"
        or s.get("replacement_material") is not None
    ):
        add(errors,f"{p} non-substantial replacement state invalid")

def relation(g,a):
    if not txt(g) or not txt(a) or "unknown" in {g,a}:
        return "unknown"
...
def risk_effect(r):
    ...
    if secondary:
        if (
            r.get("secondary_review_status")=="completed"
            and r.get("secondary_review_outcome")=="agreement"
        ):
            effect="none"
        elif (
            r.get("secondary_review_status")=="completed"
            and r.get("secondary_review_outcome")=="disagreement"
        ):
    ...
    corr=(
        {"same_family":"present","unknown":"unknown","different_family":"low"}
        .get(rel,"unknown")
...
def cardset_status(b):
    ...
    if not isinstance(audits,list):
        audits=[]
    if len(audits)<2 and not fs and cov=="NOT_APPLICABLE":
        return "NOT_APPLICABLE"
    mats=[x.get("materiality") for x in fs if isinstance(x,dict)]
    if cov=="BLOCKED" or "blocking_unverifiable" in mats:
        return "BLOCKED"
    if cov=="FAIL" or "material" in mats:
        return "FAIL"
    if cov=="WARNING" or "non_material" in mats:
        return "WARNING"
...
def release_status(b):
    ...
    if isinstance(effect,str) and effect in {
        "blocked_pending_secondary_review",
        "blocked_unresolved_disagreement",
    }:
        return "BLOCKED"
    if "BLOCK_UNVERIFIABLE" in vs or "BLOCKED" in pair:
        return "BLOCKED"
    if any(isinstance(v,str) and v in FAIL for v in vs) or "FAIL" in pair:
        return "FAIL"
    if effect=="warning_ceiling" or "PASS_WITH_WARNINGS" in vs or "WARNING" in pair:
...
def human_display(b,tickets,errors):
    ...
    fails=[
        ...
        and c.get("verdict") in FAIL
    ]
    d=b.get("human_repair_display")
    if not fails:
        if d is not None:
            add(errors,"human_repair_display must be null when no failed cards")
        return
    if not isinstance(d,dict):
        add(errors,"failed cards require human_repair_display writing block")
        return
    if d.get("surface")!="writing_block" or d.get("single_block") is not True:
        add(errors,"human_repair_display must be one writing_block")
    if d.get("first_line")!="imgedit":
        add(errors,"human_repair_display first line must be exactly imgedit")
    secs=d.get("sections",[])
    if not isinstance(secs,list):
        add(errors,"human_repair_display.sections must be array")
        return
    expected=[c.get("card_id") for c in fails]
    actual=[s.get("card_id") for s in secs if isinstance(s,dict)]
    if actual!=expected:
        add(errors,"human_repair_display sections must match failed cards in card order")
    for c,s in zip(fails,secs):
        if not isinstance(s,dict):
            add(errors,f"human repair section {c.get('card_id')} must be object")
            continue
        title,tid=c.get("card_title"),c.get("repair_ticket_id")
        t=tickets.get(tid,{}) if txt(tid) else {}
        if s.get("card_title")!=title or s.get("heading")!=f"[{title}]":
            add(errors,f"human repair section {c.get('card_id')} heading must be exact [card title]")
        if s.get("ticket_id")!=tid or s.get("content")!=t.get("human_repair_text"):
            add(errors,f"human repair section {c.get('card_id')} ticket/content mismatch")
    expected_text="imgedit"+"".join(
        f"\n[{s['card_title']}]\n{s['content']}"
        for s in secs
        if isinstance(s,dict)
    )
    if d.get("rendered_text")!=expected_text:
        add(errors,"human_repair_display.rendered_text format mismatch")
...
def validate_bundle(b:Any,*,base_dir:Path|None=None):
    ...
    if contract_version!=RESULT_CONTRACT_VERSION:
        add(e,f"contract_version must be {RESULT_CONTRACT_VERSION}")
    if b.get("result_schema_version")!=RESULT_SCHEMA_VERSION:
        add(e,f"result_schema_version must be {RESULT_SCHEMA_VERSION}")
    if b.get("policy_id")!=POLICY_ID:
        add(e,f"policy_id must be {POLICY_ID}")
    if b.get("policy_version")!=POLICY_VERSION:
        add(e,f"policy_version must be {POLICY_VERSION}")
    if b.get("method_revision")!=METHOD:
        add(e,f"method_revision must be {METHOD}")
    if b.get("policy_digest")!=POLICY_DIGEST:
        add(e,"policy_digest does not match canonical policy pack")
    ...
    raw=b.get("repair_tickets",[])
    tickets={}
    if not isinstance(raw,list):
        add(e,"repair_tickets must be array")
        raw=[]
    for t in raw:
        validate_ticket(t,e)
        if isinstance(t,dict) and txt(t.get("ticket_id")):
            tickets[t["ticket_id"]]=t
    cards=b.get("card_audits",[])
    if not isinstance(cards,list) or not cards:
        add(e,"card_audits must be non-empty")
        cards=[]
    ids=[]
    for c in cards:
        if not isinstance(c,dict):
            add(e,"card audit must be object")
            continue
        cid=c.get("card_id")
        if not txt(cid):
            add(e,"card_id must be non-empty string")
        else:
            ids.append(cid)
        p=f"card {cid}"
        if not txt(c.get("card_title")):
            add(e,f"{p} card_title required")
        if file_sha(base_dir,c.get("image_path"))!=c.get("image_sha256"):
            add(e,f"{p} image hash mismatch")
        br=c.get("blind_readback")
        if not isinstance(br,dict):
            add(e,f"{p} blind_readback must be object")
            br={}
        else:
            if br.get("frozen_before_comparison") is not True or not sha(br.get("digest")):
                add(e,f"{p} blind readback not frozen/bound")
            if br.get("digest")!=blind_payload_digest(br):
                add(e,f"{p} blind readback digest must match frozen payload")
            if not sha(br.get("visual_graph_digest")):
                add(e,f"{p} visual graph digest required")
            if not isinstance(br.get("evidence_annotation_inventory"),list):
                add(e,f"{p} evidence_annotation_inventory must be frozen as an array")
        packet=_validate_semantic_packet(c,source_bindings,p,e)
        if not sha(c.get("expected_semantic_packet_digest")):
            add(e,f"{p} expected semantic digest invalid")
        if c.get("expected_semantic_packet_digest") != expected_semantic_packet_digest(c):
            add(e,f"{p} expected_semantic_packet_digest must bind canonical semantic packet")
        fm=finding_map(c.get("findings"),f"{p}.findings",e)
        expected=visual_status(c.get("visual_semantic_reconciliation"),fm,p,e)
        ...
        if isinstance(ssr,dict):
            for group in (ssr.get("content_node_checks",[]),ssr.get("evidence_annotation_checks",[])):
                for item in group if isinstance(group,list) else []:
                    ...
                    if txt(fid):
                        if fid not in fm:
                            add(e,f"{p} source-surface finding {fid} must link to card finding")
                        elif fm[fid].get("axis")!="SOURCE_SURFACE":
                            add(e,f"{p} source-surface finding {fid} must use SOURCE_SURFACE axis")
        ...
        if source_refs and source_bindings:
            ...
            if unknown:
                add(e,f"{p} source references unknown source ids: {', '.join(sorted(unknown))}")
        rec=c.get("visual_semantic_reconciliation",{})
        if isinstance(rec,dict):
            if br.get("visual_graph_digest")!=rec.get("observed_graph_digest"):
                add(e,f"{p} frozen blind graph digest mismatch")
        axes=c.get("axes",{})
        if not isinstance(axes,dict):
            add(e,f"{p} axes must be object")
            axes={}
        axis_names=SEM+("ENGINEERING_CONFORMANCE",)
        if any(x not in axes for x in axis_names):
            add(e,f"{p} axes incomplete")
        else:
            if axes.get("VISUAL_SEMANTICS")!=expected:
                add(e,f"{p} VISUAL_SEMANTICS must be {expected} from topology reconciliation")
            if source_expected is not None and axes.get("SOURCE_SURFACE")!=source_expected:
                add(e,f"{p} SOURCE_SURFACE must be {source_expected} from source-surface reconciliation")
        ...
        if c.get("release_blocking") is not (isinstance(v,str) and v in BLOCK):
            add(e,f"{p} release_blocking mismatch")
        if v=="FAIL_SPEC" and c.get("failure_origin") not in {
            "FINAL_CARD_SPEC",
            "TRUTH_BOUNDARY",
            "SOURCE_BINDING",
            "MIXED",
        }:
            add(e,f"{p} FAIL_SPEC requires failure_origin")
        if v!="FAIL_SPEC" and c.get("failure_origin") is not None:
            add(e,f"{p} failure_origin only for FAIL_SPEC")
        tid=c.get("repair_ticket_id")
        if isinstance(v,str) and v in FAIL and (not txt(tid) or tid not in tickets):
            add(e,f"{p} fail requires repair ticket")
        if (not isinstance(v,str) or v not in FAIL) and tid is not None:
            add(e,f"{p} non-fail cannot have repair ticket")
        vals=[axes.get(x) for x in SEM]
        eng=axes.get("ENGINEERING_CONFORMANCE")
        if v=="PASS" and (any(x!="pass" for x in vals) or eng!="pass"):
            add(e,f"{p} PASS axes mismatch")
        if v=="PASS_WITH_WARNINGS" and any(x in {"fail","unverifiable"} for x in vals):
            add(e,f"{p} warning verdict contains failed/unverifiable axis")
        if v=="BLOCK_UNVERIFIABLE" and not any(x=="unverifiable" for x in vals):
            add(e,f"{p} block requires unverifiable axis")
        findings=c.get("findings",[])
        substantive_findings=findings if isinstance(findings,list) else []
        has_substantive_finding=any(
            isinstance(f,dict)
            and f.get("materiality")=="material"
            and f.get("axis")!="ENGINEERING_CONFORMANCE"
            for f in substantive_findings
        )
        if isinstance(v,str) and v in FAIL and not has_substantive_finding:
            add(e,f"{p} fail requires substantive material finding")
    if len(ids)!=len(set(ids)):
        add(e,"card ids must be unique")
    ...
    if not isinstance(dups,list):
        ...
        dups=[]
    if dd.get("unique_card_count")!=len(cards) or dd.get("duplicate_count")!=len(dups):
        add(e,"dedup counts mismatch")
    cs=b.get("cardset_audit",{})
    if not isinstance(cs,dict):
        add(e,"cardset_audit must be object")
        cs={}
    finding_map(cs.get("cross_card_findings"),"cardset findings",e)
    cexp=cardset_status({**b,"cardset_audit":cs})
    ...
    if cs.get("scope")!="cross_card_and_package_only" or cs.get("status")!=cexp:
        add(e,f"cardset status must be {cexp}")
    ...
    if r.get("relationship")!=rel:
        add(e,f"methodological relationship must be {rel}")
    material,secondary,effect,corr=risk_effect(r)
    if (
        r.get("material_to_run") is not material
        or r.get("secondary_review_required") is not secondary
        or r.get("release_effect")!=effect
        or r.get("correlated_error_risk")!=corr
    ):
        add(e,"methodological-risk derivation mismatch")
    if r.get("high_stakes_cardset") is True and not r.get("high_stakes_reasons"):
        add(e,"high-stakes run requires reasons")
    ...
    if (
        rg.get("status")!=rexp
        or rg.get("release_allowed") is not (rexp in {"PASS","PASS_WITH_WARNINGS"})
    ):
        add(e,f"release gate must be {rexp}")
    ...
    for v in ("PASS","PASS_WITH_WARNINGS","FAIL_RENDER","FAIL_SPEC","BLOCK_UNVERIFIABLE"):
        ...
        if counts.get(v)!=expected_count:
            add(e,f"release count {v} mismatch")
    ...
    if not isinstance(p,dict):
        ...
        p={}
    if (
        p.get("auditor_role")!="post_render_auditor"
        or p.get("mutated_upstream_artifacts") is not False
        or p.get("generation_calls_made")!=0
    ):
        add(e,"RCA read-only provenance violated")
    return e
...
def main():
    ...
    try:
        ...
        errors=validate_bundle(b,base_dir=a.bundle.resolve().parent)
