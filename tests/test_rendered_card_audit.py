...
class RenderedCardAuditTests(unittest.TestCase):
    ...
    def rebind_payloads(self,c):
        c["expected_semantic_packet"]["expected_graph"]=copy.deepcopy(c["visual_semantic_reconciliation"]["expected_graph"])
        c["expected_semantic_packet"]["expected_graph_digest"]=c["visual_semantic_reconciliation"]["expected_graph_digest"]
        c["blind_readback"]["digest"]=MODULE.blind_payload_digest(c.get("blind_readback",{}))
        c["expected_semantic_packet_digest"]=MODULE.expected_semantic_packet_digest(c)
        return c
    ...
    def test_topology_status_tables_are_policy_driven(self):
        b=copy.deepcopy(self.fixture)
        original=list(MODULE.EDGE_STATUSES["pass"])
        try:
            MODULE.EDGE_STATUSES["pass"].append("policy_defined_equivalent")
            b["card_audits"][0]["visual_semantic_reconciliation"]["edge_checks"][0]["status"]="policy_defined_equivalent"
            self.assertEqual(self.validate(b),[])
        finally:
            MODULE.EDGE_STATUSES["pass"][:]=original
    def test_c02_wrong_branch_is_canonical_fail_record(self): self.assertEqual(self.validate(self.c02_wrong_branch_bundle()),[])
    def test_tampered_source_binding_digest_is_rejected(self):
        b=copy.deepcopy(self.fixture)
        b["source_bindings"][0]["source_digest"]="0"*64
        self.assertTrue(any("source_bindings[0].source_digest must match artifact bytes" in e for e in self.validate(b)))
    def test_tampered_source_binding_metadata_is_rejected(self):
        b=copy.deepcopy(self.fixture)
        b["source_bindings"][0]["identity"]["exact_title"]="Different source"
        self.assertTrue(any("source_bindings[0].binding_digest must bind" in e for e in self.validate(b)))
    def test_legacy_contract_version_is_rejected(self):
        b=copy.deepcopy(self.fixture); b["contract_version"]="1.0"
        self.assertTrue(any("contract_version must be 1.2.0" in e for e in self.validate(b)))
    def test_tampered_policy_digest_is_rejected(self):
        b=copy.deepcopy(self.fixture); b["policy_digest"]="0"*64
        self.assertTrue(any("policy_digest does not match canonical policy pack" in e for e in self.validate(b)))
    ...
    def test_tampered_blind_readback_digest_is_rejected(self):
        b=copy.deepcopy(self.fixture)
        b["card_audits"][0]["blind_readback"]["digest"]="0"*64
        self.assertTrue(any("blind readback digest must match frozen payload" in e for e in self.validate(b)))
    def test_tampered_expected_semantic_packet_digest_is_rejected(self):
        b=copy.deepcopy(self.fixture)
        b["card_audits"][0]["expected_semantic_packet_digest"]="0"*64
        self.assertTrue(any("expected_semantic_packet_digest must bind canonical semantic packet" in e for e in self.validate(b)))
    def test_expected_packet_graph_must_match_reconciliation(self):
        b=copy.deepcopy(self.fixture)
        b["card_audits"][0]["expected_semantic_packet"]["expected_graph_digest"]="0"*64
        b["card_audits"][0]["expected_semantic_packet_digest"]=MODULE.expected_semantic_packet_digest(b["card_audits"][0])
        self.assertTrue(any("expected semantic packet graph digest mismatch" in e for e in self.validate(b)))
    def test_expected_packet_source_binding_must_resolve(self):
        b=copy.deepcopy(self.fixture)
        b["card_audits"][0]["expected_semantic_packet"]["source_binding_ids"]=["SRC_UNKNOWN"]
        b["card_audits"][0]["expected_semantic_packet_digest"]=MODULE.expected_semantic_packet_digest(b["card_audits"][0])
        self.assertTrue(any("expected semantic packet references unknown source ids" in e for e in self.validate(b)))
    def test_expected_packet_source_binding_digest_is_bound(self):
        b=copy.deepcopy(self.fixture)
        b["card_audits"][0]["expected_semantic_packet"]["source_binding_digests"]["SRC001"]="0"*64
        b["card_audits"][0]["expected_semantic_packet_digest"]=MODULE.expected_semantic_packet_digest(b["card_audits"][0])
        self.assertTrue(any("expected semantic packet source binding digest mismatch" in e for e in self.validate(b)))
    def test_expected_packet_graph_object_is_bound_to_reconciliation(self):
        b=copy.deepcopy(self.fixture)
        b["card_audits"][0]["expected_semantic_packet"]["expected_graph"]["nodes"][0]["label"]="tampered"
        b["card_audits"][0]["expected_semantic_packet"]["expected_graph_digest"]=MODULE.jd(b["card_audits"][0]["expected_semantic_packet"]["expected_graph"])
        b["card_audits"][0]["expected_semantic_packet_digest"]=MODULE.expected_semantic_packet_digest(b["card_audits"][0])
        self.assertTrue(any("does not equal reconciliation graph" in e for e in self.validate(b)))
    def test_expected_packet_graph_requires_full_object_lists(self):
        b=copy.deepcopy(self.fixture)
        b["card_audits"][0]["expected_semantic_packet"]["expected_graph"].pop("relations")
        b["card_audits"][0]["expected_semantic_packet_digest"]=MODULE.expected_semantic_packet_digest(b["card_audits"][0])
        self.assertTrue(any("expected_graph.relations must be array" in e for e in self.validate(b)))
    def test_non_object_blind_readback_fails_without_exception(self):
        b=copy.deepcopy(self.fixture); b["card_audits"][0]["blind_readback"]="invalid"
        self.assertTrue(any("blind_readback must be object" in e for e in self.validate(b)))
    def test_unknown_source_locator_is_rejected(self):
        b=copy.deepcopy(self.fixture)
        c=b["card_audits"][0]
        c["source_surface_reconciliation"]["content_node_checks"][0]["source_locators"]=["SRC_UNKNOWN:results"]
        self.assertTrue(any("source references unknown source ids: SRC_UNKNOWN" in e for e in self.validate(b)))
    def test_visual_source_locator_is_part_of_source_closure(self):
        b=copy.deepcopy(self.fixture)
        b["card_audits"][0]["visual_semantic_reconciliation"]["source_structure"]["source_locators"]=["SRC_UNKNOWN:figure"]
        self.assertTrue(any("source references unknown source ids: SRC_UNKNOWN" in e for e in self.validate(b)))
    def test_source_locator_must_use_source_id_locator_form(self):
        b=copy.deepcopy(self.fixture)
        b["card_audits"][0]["visual_semantic_reconciliation"]["source_structure"]["source_locators"]=["SRC001"]
        self.assertTrue(any("source_id:locator form" in e for e in self.validate(b)))
    def test_unknown_verdict_fails_closed(self):
        b=copy.deepcopy(self.fixture)
        c=b["card_audits"][0]
        c.update({"verdict":"NOT_A_VERDICT","release_blocking":False,"repair_ticket_id":None})
        b.update({"repair_tickets":[],"human_repair_display":None})
        b["release_gate"].update({"status":"PASS","release_allowed":True})
        b["release_gate"]["counts"].update({"PASS":0,"PASS_WITH_WARNINGS":0,"FAIL_RENDER":0,"FAIL_SPEC":0,"BLOCK_UNVERIFIABLE":0})
        self.assertTrue(any("verdict invalid" in e for e in self.validate(b)))
    def test_unknown_axis_statuses_fail_closed(self):
        b=copy.deepcopy(self.fixture)
        c=b["card_audits"][0]
        for axis in MODULE.SEM+("ENGINEERING_CONFORMANCE",):
            c["axes"][axis]="NOT_A_STATUS"
        self.assertTrue(any("status invalid" in e for e in self.validate(b)))
    def test_unknown_package_coverage_status_fails_closed(self):
        for invalid in ("NOT_A_STATUS",[],{}):
            with self.subTest(invalid=invalid):
                b=copy.deepcopy(self.fixture)
                b["cardset_audit"]["package_coverage"]["status"]=invalid
                self.assertTrue(any("package_coverage.status invalid" in e for e in self.validate(b)))
    def test_malformed_blind_inventory_item_fails_without_exception(self):
        b=copy.deepcopy(self.fixture)
        c=b["card_audits"][0]
        c["blind_readback"]["content_node_inventory"].append("not-an-object")
        c["blind_readback"]["digest"]=MODULE.blind_payload_digest(c["blind_readback"])
        errors=self.validate(b)
        self.assertTrue(any("content_node_inventory[4] must be object" in e for e in errors))
    def test_malformed_observed_graph_node_fails_without_exception(self):
        b=copy.deepcopy(self.fixture)
        c=b["card_audits"][0]
        rec=c["visual_semantic_reconciliation"]
        rec["observed_graph"]["nodes"]=["not-an-object"]
        rec["observed_graph_digest"]=MODULE.jd(rec["observed_graph"])
        c["blind_readback"]["visual_graph_digest"]=rec["observed_graph_digest"]
        c["blind_readback"]["digest"]=MODULE.blind_payload_digest(c["blind_readback"])
        errors=self.validate(b)
        self.assertTrue(any("observed graph nodes[0] must be object" in e for e in errors))
    def test_source_binding_type_status_and_identity_are_required(self):
        for field in ("type","status","identity"):
            with self.subTest(field=field):
                b=copy.deepcopy(self.fixture)
                b["source_bindings"][0].pop(field)
                self.assertTrue(any(f"source_bindings[0].{field}" in e for e in self.validate(b)))
    def test_source_binding_digest_covers_unrecognised_metadata(self):
        b=copy.deepcopy(self.fixture)
        b["source_bindings"][0]["unrecognised_metadata"]="must be bound"
        self.assertTrue(any("binding_digest must bind" in e for e in self.validate(b)))
    def test_symlink_artifact_escape_is_rejected(self):
        import tempfile
        with tempfile.TemporaryDirectory(dir=ROOT) as td, tempfile.TemporaryDirectory() as od:
            outside=Path(od)/"outside.txt"
            outside.write_text("outside",encoding="utf-8")
            link=Path(td)/"escape.txt"
            link.symlink_to(outside)
            self.assertIsNone(MODULE.file_sha(Path(td),"escape.txt"))
    ...
    def test_malformed_nested_objects_fail_without_exception(self):
        paths=(
            ("dedup_manifest",[]),
            ("cardset_audit",[]),
            ("methodological_risk",[]),
        ...
        for field,value in paths:
            with self.subTest(field=field):
                b=copy.deepcopy(self.fixture); b[field]=value
                errors=self.validate(b)
